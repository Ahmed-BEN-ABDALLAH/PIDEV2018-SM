...
class ProfilJDRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    function findadresse($p)
    {
        $query=$this->getEntityManager()->createQuery(
            "select j from JardinDenfantProfilJDBundle:ProfilJD j where j.adresse like :p
or j.nomJD LIKE :p
")->setParameter('p','%'.$p.'%');
    ...
    select count(j) AS  nb from JardinDenfantProfilJDBundle:ProfilJD j")
        ...
        public function findNotValide()
        {
            $query=$this->getEntityManager()->createQuery(
                "select m from JardinDenfantProfilJDBundle:ProfilJD m where m.valide =0  ");
            return $query->getResult();
        }
    ...
    {
        $query=$this->getEntityManager()->createQuery(
            "select m from JardinDenfantProfilJDBundle:ProfilJD m where m.id =:id")
            ->setParameter('id',$id)
            ->setMaxResults(1);
