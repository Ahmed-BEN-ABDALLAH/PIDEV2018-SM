...
class ForumController extends Controller
{
    ...
    public function Consulter_publicationforumAction(Request $request , $id_publication)
    {
        ...
        if ($formreponse->isSubmitted() && $formreponse->isValid() && $request->getMethod()=='POST') {
            ...
            $newreponse->setIdPublication($publicationaffiche);
    ...
    {
        $em=$this->getDoctrine()->getManager();
        $publicationrecents= $em->getRepository('ForumBundle:Publication')->findBy(['location' => 'approuver']);
        ...
        $formatted = $serializer->normalize($publicationrecents);
        return new JsonResponse($formatted);
    }
    public function allallcategAction()
    {
        $em=$this->getDoctrine()->getManager();
        $publicationrecents= $em->getRepository('ForumBundle:Categorie')->findAll();
    ...
    public function findCommentssAction($id)
    {
        $em=$this->getDoctrine()->getManager();
        $reps = $em->getRepository('ForumBundle:Reponse')->findBy(['id_publication' => $id]);
        ...
        $normalizers = array($normalizer);
        $serializer = new Serializer($normalizers,[new ObjectNormalizer()]);
        $formatted = $serializer->normalize($reps);
        return new JsonResponse($formatted);

    }
    public function ajoutpubjsAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $emm = $this->getDoctrine()->getManager();
        $emmm = $this->getDoctrine()->getManager();
        $task = new Publication();
        $task->setIddcategorie($request->get('categorie'));
        $task->setU($request->get('user'));
        $task->setTitre($request->get('titre'));
        $ius=$request->get('user');
        $ict=$request->get('categorie');
        $task->setNbrVue($request->get('vues'));
        $date=date_create();
        $task->setCreated(date_timestamp_get($date));
        $task->setLocation($request->get('location'));
        $task->setContenu($request->get('contenu'));
        $task->setNbrReponse($request->get('reponses'));
        $em->persist($task);
        $em->flush();
        $us=$emm->getRepository('UserBundle:User')->find($ius);
        $task->setIduser($us);
        $emm->flush();
        $ct=$emm->getRepository('ForumBundle:Categorie')->find($ict);
        $task->setIdCategorie($ct);
        $emmm->flush();
    ...
    public function ajoutrepjsAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $emm = $this->getDoctrine()->getManager();
        $emmm = $this->getDoctrine()->getManager();
        $task = new Reponse();
        $task->setPubid($request->get('publication'));
        $task->setUsrid($request->get('user'));
        $task->setCreated(new \DateTime("now"));
        $ius=$request->get('user');
        $ipb=$request->get('publication');
        $task->setContenu($request->get('contenu'));
        $em->persist($task);
        $em->flush();
        $us=$emm->getRepository('UserBundle:User')->find($ius);
        ...
        $emm->persist($task);
        $emm->flush();
        $pb=$emm->getRepository('ForumBundle:Publication')->find($ipb);
        $task->setIdPublication($pb);
        $emmm->persist($task);
        $emmm->flush();
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($task);
